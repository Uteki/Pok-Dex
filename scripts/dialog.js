    ...
    monsters.some(monster => {
        if (monster.id === id) {
            document.querySelector(".modal-dialog").innerHTML += dialogTemplate(
                ...
                monster.height, monster.weight, monster.cries, monster.captureRate, monster.species
            )
        }
    })
    myModal.show();
...
function translateStuff(data) {
    ...
    return [latestName, latestDescription, latestSpecies, captureRate];
}
